...
export default function BlogCard({ post }: { post: BlogPost }) {
  return (
    <div className="group relative h-full">
      {/* Outer Glow on Hover */}
      <div className="absolute -inset-1 bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 rounded-2xl blur opacity-0 group-hover:opacity-40 group-hover:blur-sm transition-all duration-500"></div>
      
      {/* Main Card */}
      <div className="relative h-full bg-white rounded-2xl shadow-sm border border-gray-100 
                      group-hover:shadow-2xl group-hover:shadow-indigo-100 
                      group-hover:scale-[1.02] transition-all duration-300 will-change-transform">
